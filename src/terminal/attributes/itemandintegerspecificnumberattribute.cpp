...
QStringList ItemAndIntegerSpecificNumberAttribute::set(const QStringList ids, const QHash<Keys::Key, QStringList> attributes, QList<Keys::Key> valueKeys) {
                ...
                if (removeValues) {
                    ...
                    if (difference) {
                        ...
                        if (!currentValueQuery.exec()) {
                            ...
                            output.append(Terminal::formatWarningMessage("Failed loading the current " + name + " of " + item.getSingular() + " " + id + " and " + foreignItem.getSingular() + " " + foreignItemId + "."));
                            valueOk = false;
                        } else {
                    ...
                    if (valueOk) {
                        const QVariant formattedValue = keyNumber.format(value);
                        if (formattedValue.isValid()) {
                            ...
                            if (!query.exec()) {
                                allQueriesSuccessful = false;
                                qWarning() << Q_FUNC_INFO << query.executedQuery() << query.lastError().text();
                                output.append(Terminal::formatErrorMessage("Failed updating the " + name + " of " + item.getSingular() + " " + id + " and " + foreignItem.getSingular() + " " + foreignItemId + "."));
                            }
                        } else {
    ...
    if (!successfulIds.isEmpty()) {
        if (removeValues) {
            output.append(Terminal::formatSuccessMessage("Removed " + name + " of " + item.format(successfulIds) + " at " + foreignItem.format(foreignItemIds) + " and " + key.toString() + keyNumber.getUnit() + "."));
        } else if (difference) {
            output.append(Terminal::formatSuccessMessage("Changed " + name + " of " + item.format(successfulIds) + " at " + foreignItem.format(foreignItemIds) + " and " + key.toString() + keyNumber.getUnit() + " by " + Keys::keysToString(valueKeys) + valueNumber.getUnit() + "."));
        } else {
            output.append(Terminal::formatSuccessMessage("Set " + name + " of " + item.format(successfulIds) + " at " + foreignItem.format(foreignItemIds) + " and " + key.toString() + keyNumber.getUnit() + " to " + Keys::keysToString(valueKeys) + valueNumber.getUnit() + "."));
        }
    }
