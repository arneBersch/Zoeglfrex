...
int main(int argc, char *argv[]) {
    ...
    if (fileExists) {
        ...
        const QStringList createTableQueries = {
            ...
            "CREATE TABLE raws (key INTEGER, id TEXT UNIQUE NOT NULL, sortkey INTEGER NOT NULL, label TEXT DEFAULT '', PRIMARY KEY (key))",
            ...
            "CREATE TABLE intensity_model_dimmer (intensity_key INTEGER REFERENCES intensities (key) ON DELETE CASCADE, model_key INTEGER REFERENCES models (key) ON DELETE CASCADE, dimmer REAL DEFAULT 0, PRIMARY KEY (intensity_key, model_key))",
            "CREATE TABLE intensity_fixture_dimmer (intensity_key INTEGER REFERENCES intensities (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, dimmer REAL DEFAULT 0, PRIMARY KEY (intensity_key, fixture_key))",
            "CREATE TABLE intensity_raws (intensity_key INTEGER REFERENCES intensities (key) ON DELETE CASCADE, raw_key INTEGER REFERENCES raws (key) ON DELETE CASCADE, PRIMARY KEY (intensity_key, raw_key))",
            ...
            "CREATE TABLE color_model_hue (color_key INTEGER REFERENCES colors (key) ON DELETE CASCADE, model_key INTEGER REFERENCES models (key) ON DELETE CASCADE, hue REAL DEFAULT 0, PRIMARY KEY (color_key, model_key))",
            "CREATE TABLE color_fixture_hue (color_key INTEGER REFERENCES colors (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, hue REAL DEFAULT 0, PRIMARY KEY (color_key, fixture_key))",
            "CREATE TABLE color_model_saturation (color_key INTEGER REFERENCES colors (key) ON DELETE CASCADE, model_key INTEGER REFERENCES models (key) ON DELETE CASCADE, saturation REAL DEFAULT 0, PRIMARY KEY (color_key, model_key))",
            "CREATE TABLE color_fixture_saturation (color_key INTEGER REFERENCES colors (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, saturation REAL DEFAULT 0, PRIMARY KEY (color_key, fixture_key))",
            "CREATE TABLE color_raws (color_key INTEGER REFERENCES colors(key) ON DELETE CASCADE, raw_key INTEGER REFERENCES raws (key) ON DELETE CASCADE, PRIMARY KEY (color_key, raw_key))",
            ...
            "CREATE TABLE position_model_pan  (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, model_key INTEGER REFERENCES models (key) ON DELETE CASCADE, pan REAL DEFAULT 0, PRIMARY KEY (position_key, model_key))",
            "CREATE TABLE position_fixture_pan (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, pan REAL DEFAULT 0, PRIMARY KEY (position_key, fixture_key))",
            "CREATE TABLE position_model_tilt (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, model_key INTEGER REFERENCES models (key) ON DELETE CASCADE, tilt REAL DEFAULT 0, PRIMARY KEY (position_key, model_key))",
            "CREATE TABLE position_fixture_tilt (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, tilt REAL DEFAULT 0, PRIMARY KEY (position_key, fixture_key))",
            "CREATE TABLE position_model_zoom (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, model_key INTEGER REFERENCES models (key) ON DELETE CASCADE, zoom REAL DEFAULT 15, PRIMARY KEY (position_key, model_key))",
            "CREATE TABLE position_fixture_zoom (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, zoom REAL DEFAULT 15, PRIMARY KEY (position_key, fixture_key))",
            "CREATE TABLE position_model_focus (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, model_key INTEGER REFERENCES models (key) ON DELETE CASCADE, focus REAL DEFAULT 0, PRIMARY KEY (position_key, model_key))",
            "CREATE TABLE position_fixture_focus (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, focus REAL DEFAULT 0, PRIMARY KEY (position_key, fixture_key))",
            "CREATE TABLE position_raws (position_key INTEGER REFERENCES positions (key) ON DELETE CASCADE, raw_key INTEGER REFERENCES raws (key) ON DELETE CASCADE, PRIMARY KEY (position_key, raw_key))",
            ...
            "CREATE TABLE cues (key INTEGER, id TEXT UNIQUE NOT NULL, sortkey INTEGER NOT NULL, label TEXT DEFAULT '', PRIMARY KEY (key))",
            ...
            "CREATE TABLE cue_raws (cue_key INTEGER REFERENCES cues (key) ON DELETE CASCADE, group_key INTEGER REFERENCES groups (key) ON DELETE CASCADE, raw_key INTEGER REFERENCES raws (key) ON DELETE CASCADE, PRIMARY KEY (cue_key, group_key, raw_key))",
            "CREATE TABLE cue_effects (cue_key INTEGER REFERENCES cues (key) ON DELETE CASCADE, group_key INTEGER REFERENCES groups (key) ON DELETE CASCADE, effect_key INTEGER REFERENCES effects (key) ON DELETE CASCADE, PRIMARY KEY (cue_key, group_key, effect_key))",
