...
int main(int argc, char *argv[]) {
    ...
    QStringList createTableQueries = {
        ...
        "CREATE TABLE IF NOT EXISTS fixtures (key INTEGER, id INTEGER UNIQUE NOT NULL, label TEXT DEFAULT '' NOT NULL, model INTEGER REFERENCES models (key) ON DELETE SET NULL, universe INTEGER NOT NULL DEFAULT 1, address INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (key))",
        "CREATE TABLE IF NOT EXISTS groups (key INTEGER, id INTEGER UNIQUE NOT NULL, label TEXT DEFAULT '' NOT NULL, PRIMARY KEY (key))",
        "CREATE TABLE IF NOT EXISTS intensities (key INTEGER, id INTEGER UNIQUE NOT NULL, label TEXT DEFAULT '' NOT NULL, dimmer REAL DEFAULT 0 NOT NULL, PRIMARY KEY (key), CHECK (dimmer BETWEEN 0 AND 100))",
        "CREATE TABLE IF NOT EXISTS colors (key INTEGER, id INTEGER UNIQUE NOT NULL, label TEXT DEFAULT '' NOT NULL, hue REAL DEFAULT 0 NOT NULL, saturation REAL DEFAULT 0 NOT NULL, PRIMARY KEY (key), CHECK (hue BETWEEN 0 AND 360), CHECK (saturation BETWEEN 0 AND 100))",
        "CREATE TABLE IF NOT EXISTS positions (key INTEGER, id INTEGER UNIQUE NOT NULL, label TEXT DEFAULT '' NOT NULL, PRIMARY KEY (key))",
