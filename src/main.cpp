...
int main(int argc, char *argv[]) {
    ...
    const QStringList createTableQueries = {
        ...
        "CREATE TABLE IF NOT EXISTS intensity_fixture_dimmers (intensity_key INTEGER REFERENCES intensities (key) ON DELETE CASCADE, fixture_key INTEGER REFERENCES fixtures (key) ON DELETE CASCADE, dimmer REAL DEFAULT 0, PRIMARY KEY (intensity_key, fixture_key))",
